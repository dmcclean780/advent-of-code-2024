...
def walkPath(position, value, foundNines):
    ...
    if(value == 8):
        if(up[0]>-1 and map[up[0]][up[1]] == 9 ):
            nines+=1
            foundNines.append(up)
        if(down[0]<len(map) and map[down[0]][down[1]] == 9 ):
            nines+=1
            foundNines.append(down)
        if(left[1]>-1 and map[left[0]][left[1]] == 9 ):
            nines+=1
            foundNines.append(left)
        if(right[1]<len(map[0]) and map[right[0]][right[1]] == 9 ):
            nines+=1
